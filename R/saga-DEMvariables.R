...
library(plotKML)
library(raster)
a <- readRDS('dat/SPCG2016_covs100m.rds')
b <- brick(a)
library(rgdal)
writeGDAL(a["DEMNED6_100m"], "dat/DEMNED6_100m.sdat", "SAGA")
saga_DEM_derivatives("dat/DEMNED6_100m.sgrd")
dirdir()
setwd('dat')
dem.lst <- list.files(pattern=glob2rx("^DEMNED6_*.sdat"))
plot(stack(dem.lst))
setwd('..')
system(paste0(saga_cmd, ' grid_tools 25 -GRID=\"', "DEMNED6_100m.sgrd", '\" -MASK=\"', NULL, '\" -CLOSED=\"', "DEMNED6_100m.sgrd", '\"'))
